...
class BurstModulation final : public autd3::Modulation<BurstModulation> {
 public:
  [[nodiscard]] std::vector<autd3::EmitIntensity> calc(
      const autd3::Geometry&) const override {
    std::vector<autd3::EmitIntensity> buffer(
        _buf_size, std::numeric_limits<autd3::EmitIntensity>::min());
    buffer[_buf_size - 1] = std::numeric_limits<autd3::EmitIntensity>::max();
    return buffer;
  }

  explicit BurstModulation(
      const size_t buf_size = 4000,
      const autd3::native_methods::SamplingConfigWrap config =
          autd3::SamplingConfig::Freq(4000 * autd3::Hz)) noexcept
...
class MyUniformGain final : public autd3::Gain<MyUniformGain> {
 public:
  ...
  [[nodiscard]] std::unordered_map<size_t, std::vector<autd3::Drive>> calc(
      const autd3::Geometry& geometry) const override {
    return transform(
        geometry, [](const autd3::Device&, const autd3::Transducer&) {
          return autd3::Drive{autd3::Phase(0),
