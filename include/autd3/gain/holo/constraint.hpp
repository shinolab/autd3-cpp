...
class EmissionConstraint final {
 public:
  ...
  AUTD3_API static inline native_methods::EmissionConstraintWrap Uniform(const driver::EmitIntensity value) noexcept {
    return native_methods::AUTDGainHoloConstraintUniform(value.value());
  }
  AUTD3_API static inline native_methods::EmissionConstraintWrap Clamp(const driver::EmitIntensity min_v,
                                                                       const driver::EmitIntensity max_v) noexcept {
    return native_methods::AUTDGainHoloConstraintClamp(min_v.value(), max_v.value());
  }
  AUTD3_API static inline native_methods::EmissionConstraintWrap Multiply(const double v) noexcept {
