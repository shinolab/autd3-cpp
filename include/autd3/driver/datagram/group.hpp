...
struct Group final : Datagram {
  ...
  AUTD3_API [[nodiscard]] native_methods::DatagramPtr ptr(const geometry::Geometry& geometry) const override {
    ...
    const auto f_native = +[](const void* context, const native_methods::GeometryPtr geometry_ptr, const uint16_t dev_idx) -> int32_t {
      auto* self = static_cast<Group<F>*>(const_cast<void*>(context));
 ...
 private:
  F _keymap_f;
  std::unordered_map<typename std::invoke_result_t<F, const driver::geometry::Device&>::value_type, std::shared_ptr<driver::Datagram>> _data_map;
  mutable std::unordered_map<typename std::invoke_result_t<F, const driver::geometry::Device&>::value_type, int32_t> _keymap = {};
};
