...
class ReadsFPGAState final {
  using native_f = bool (*)(const void*, native_methods::GeometryPtr, uint32_t);

 public:
  explicit ReadsFPGAState(F f) : _f(std::move(f)) {
  ...
  [[nodiscard]] native_methods::DatagramPtr ptr(const geometry::Geometry& geometry) const {
    return AUTDDatagramReadsFPGAState(const_cast<void*>(reinterpret_cast<const void*>(_f_native)), const_cast<void*>(static_cast<const void*>(this)),
                                      geometry.ptr());
  }
