...
struct ControlPoint {
  ...
  AUTD3_DEF_PROP(Point3, point)
  AUTD3_DEF_PARAM_PHASE(ControlPoint, phase_offset)

  explicit ControlPoint(Point3 point) : ControlPoint(std::move(point), Phase(0)) {}

 private:
  ControlPoint() : ControlPoint(Point3::origin()) {}
  explicit ControlPoint(Point3 point, const Phase offset) : _point(std::move(point)), _phase_offset(offset) {}
};
...
struct ControlPoints {
 ...
 public:
  ...
  explicit ControlPoints(const std::array<Point3, N>& points) : ControlPoints(points, std::numeric_limits<EmitIntensity>::max()) {}
 ...
 private:
  ...
  explicit ControlPoints(const std::array<Point3, N>& points, const EmitIntensity intensity) : _intensity(intensity) {
...
struct ControlPoints<1> {
 ...
 public:
  ...
  explicit ControlPoints(const std::array<Point3, 1>& points) : ControlPoints(points, std::numeric_limits<EmitIntensity>::max()) {}
  explicit ControlPoints(Point3 point) : ControlPoints({std::move(point)}, std::numeric_limits<EmitIntensity>::max()) {}
 ...
 private:
  ...
  explicit ControlPoints(const std::array<Point3, 1>& points, const EmitIntensity intensity) : _intensity(intensity) {
