...
class ModulationBase
    : public DatagramST<native_methods::ModulationPtr>,
      public IntoDatagramWithSegmentTransition<native_methods::ModulationPtr,
                                               M> {
 public:
  ...
  ModulationBase& operator=(const ModulationBase& obj) =
  ...
  AUTD3_API [[nodiscard]] native_methods::ModulationPtr raw_ptr(
  ...
  AUTD3_API [[nodiscard]] native_methods::DatagramPtr into_segment(
      const native_methods::ModulationPtr p,
  ...
  AUTD3_API [[nodiscard]] native_methods::DatagramPtr into_segment_transition(
    ...
    return AUTDModulationIntoDatagramWithSegmentTransition(p, segment,
                                                           transition_mode);
  }
  AUTD3_API [[nodiscard]] virtual native_methods::ModulationPtr modulation_ptr(
      const geometry::Geometry&) const = 0;

  void with_loop_behavior(const native_methods::LoopBehavior loop_behavior) & {
    _loop_behavior = loop_behavior;
  }
  AUTD3_API [[nodiscard]] M&& with_loop_behavior(
      const native_methods::LoopBehavior loop_behavior) && {
    _loop_behavior = loop_behavior;
    return std::move(*static_cast<M*>(this));
  }

  AUTD3_API [[nodiscard]] native_methods::LoopBehavior loop_behavior()
      const noexcept {
    return _loop_behavior;
  }
