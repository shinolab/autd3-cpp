...
struct STMSamplingConfig final {
  ...
  static STMSamplingConfig nearest(const driver::Freq<float> freq, const uint16_t n) {
  ...
  explicit STMSamplingConfig(driver::SamplingConfig config, const uint16_t n) : _inner(std::move(config)), _n(n) {}
  ...
  explicit STMSamplingConfig(STMSamplingConfig config, const uint16_t n) : _inner(std::move(config._inner)), _n(n) {}

 private:
  explicit STMSamplingConfig(native_methods::SamplingConfig config, const uint16_t n) : _inner(std::move(config)), _n(n) {}
