...
TEST(Modulation, RawPCMResampler) {
  auto autd = create_controller();

  const std::filesystem::path path = std::filesystem::path(AUTD3_TEST_AUDIOFILE_RESOURCE_PATH).append("custom.dat");
  autd.send(autd3::modulation::audio_file::RawPCM(path, 2.0f * autd3::driver::kHz, 4u * autd3::driver::kHz, autd3::modulation::SincInterpolation()));
