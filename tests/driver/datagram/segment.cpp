...
TEST(DriverDatagram, SwapSegmentGain) {
  ...
  autd.send(autd3::driver::WithSegment(g, Segment::S1, autd3::driver::transition_mode::Immediate()));
  infos = autd.fpga_state();
  for (auto& dev : autd) {
    ASSERT_EQ(std::nullopt, infos[dev.idx()].value().current_stm_segment());
  ...
  autd.send(autd3::driver::WithSegment(autd3::gain::Uniform(autd3::driver::Intensity(0x82), autd3::driver::Phase(0x92)), Segment::S0,
                                       autd3::driver::transition_mode::Later()));
  infos = autd.fpga_state();
  for (auto& dev : autd) {
    ASSERT_EQ(std::nullopt, infos[dev.idx()].value().current_stm_segment());
    ASSERT_EQ(Segment::S1, infos[dev.idx()].value().current_gain_segment());
...
TEST(DriverDatagram, SwapSegmentModulation) {
  ...
  autd.send(autd3::driver::WithSegment(m, Segment::S1, autd3::driver::transition_mode::Immediate()));
  infos = autd.fpga_state();
  for (auto& dev : autd) {
