...
TEST(DriverDatagramModulation, Cache) {
  ...
  const auto m1 = autd3::modulation::Static::with_intensity(
      autd3::driver::EmitIntensity(0x80));
  const auto m2 = autd3::modulation::Static::with_intensity(
                      autd3::driver::EmitIntensity(0x80))
                      .with_cache();
  ...
  ASSERT_TRUE(std::ranges::all_of(m2.buffer(),
                                  [](auto d) { return d.value() == 0x80; }));
  for (const auto& m : m2) ASSERT_EQ(0x80, m.value());
  std::ranges::for_each(m2.cbegin(), m2.cend(),
                        [](const auto& m) { ASSERT_EQ(0x80, m.value()); });
  for (size_t i = 0; i < 2; i++) ASSERT_EQ(0x80, m2[i].value());
  for (auto& dev : autd1.geometry()) {
    auto mod =
        autd2.link().modulation(dev.idx(), autd3::native_methods::Segment::S0);
    auto mod_expect =
        autd1.link().modulation(dev.idx(), autd3::native_methods::Segment::S0);
    ASSERT_TRUE(std::ranges::equal(mod, mod_expect));
    ASSERT_EQ(0xFFFFFFFF, autd2.link().modulation_freq_division(
                              dev.idx(), autd3::native_methods::Segment::S0));
  }
}

class ForModulationCacheTest final
    : public autd3::modulation::Modulation<ForModulationCacheTest> {
 public:
  AUTD3_API [[nodiscard]] std::vector<autd3::driver::EmitIntensity> calc(
    ...
    ++*_cnt;
    return {std::numeric_limits<autd3::driver::EmitIntensity>::max(),
            std::numeric_limits<autd3::driver::EmitIntensity>::max()};
  }

  explicit ForModulationCacheTest(size_t* cnt) noexcept
      : Modulation(autd3::driver::SamplingConfig::Division(5120)), _cnt(cnt) {}
