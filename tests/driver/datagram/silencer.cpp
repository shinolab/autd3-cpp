...
TEST(DriverDatagram, Silencer) {
  ...
  autd.send(autd3::driver::Silencer(
      ...
      autd3::native_methods::SilencerTarget::Intensity));
  for (auto& dev : autd) {
  ...
  autd.send(autd3::driver::Silencer(autd3::driver::FixedUpdateRate{.intensity = 10, .phase = 20}, autd3::native_methods::SilencerTarget::Intensity));
  for (auto& dev : autd) {
...
TEST(DriverDatagram, SilencerIsDefault) {
  ...
  ASSERT_TRUE(autd3::native_methods::AUTDDatagramSilencerFixedCompletionStepsIsDefault(std::get<0>(default_.config), default_.target));
}
