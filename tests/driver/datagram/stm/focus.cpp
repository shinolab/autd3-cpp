...
TEST(DriverDatagramSTM, FocusSTM) {
  ...
  auto stm = autd3::driver::FocusSTM::from_freq(1.0 * autd3::driver::Hz).add_foci_from_iter(
...
TEST(DriverDatagramSTM, FocusSTMSegment) {
  ...
  ASSERT_TRUE(autd.send(autd3::driver::FocusSTM::from_freq(1.0 * autd3::driver::Hz)
  ...
  ASSERT_TRUE(autd.send(autd3::driver::SwapSegment::focus_stm(Segment::S0, autd3::driver::TransitionMode::Immediate)));
  infos = autd.fpga_info();
  for (auto& dev : autd.geometry()) {
    ASSERT_EQ(std::nullopt, infos[dev.idx()].value().current_gain_segment());
    ASSERT_EQ(Segment::S0, infos[dev.idx()].value().current_stm_segment());
